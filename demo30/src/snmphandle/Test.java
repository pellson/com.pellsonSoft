...
/**
 * 可以同时运行snmpget和snmpwark
 * 每种设备的oid是不同的，Linux上也要自己的oid
 * 目前成功的有：本机物理总内存，系统运行的进程列表
 * 注意这种方式前面没有点，很多在SNMPManage中使用的oid，这里不起作用
* @author 赵志蓬 E-mail:13732904468@139.com
* @version 创建时间：2017年4月13日 下午3:32:09
* @Description：
 */
public class Test {
	//private static final String IP="198.9.1.190";
	private static final String IP="192.168.1.53";
...
//			map.put("1.3.6.1.2.1.1.1", "系统描述");
			...
			map.put("1.3.6.1.2.1.25.4.2.1.2", "系统运行的进程列表");//windows下正确,但注意前面没有"点"
			map.put("1.3.6.1.2.1.1.5.0", "监控时间");
...
//			map.put("1.3.6.1.2.1.25.3.3.1.2.1","CPU使用率");
			...
			while(it.hasNext()){
				...
				i++;				
				String[] sysDescs = mgr.snmpWalk(IP, COMMUNITY,oid);
				//String sysDescs1=mgr.snmpGet(IP, COMMUNITY, oid);
				//System.out.println("sysDescs1get:"+sysDescs1);
