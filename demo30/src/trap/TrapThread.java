 ...
 * 成功输出;
 * 到时候,解析其它类型的结果可以参照这种模式   (SSH、cli、客户端exec)
 * trap正常的例子,核心
 */
...
public class TrapThread {
	...
	public static void TrapActionRun() {
		try {
			...
			String IP = "192.168.1.51";//inet.getHostAddress();
			//IP="198.9.1.191";
			// snmp4j通过transportmapping的监听端口接收SNMP信息,所以这里初始化一个transportmapping
			// 注明本机(管理端manage的ip,这里就是部署代码端的ip地址,也就是在agent的trap列表中添加的ip地址)的IP地址及接收trap的端口.
			TransportMapping transport = new DefaultUdpTransportMapping(
					new UdpAddress(IP + "/162"));//ps:这个就相当于本机中的trap服务器,所以运行程序时,trap服务器要关闭
			// 创建一个处理消息的snmp实例
